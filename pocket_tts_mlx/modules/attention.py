...
def complete_kv(
    ...
    Args:
        ...
        current_end: Current end position tensor (array where position is determined by shape[0]).
    ...
    """
    # Get the current write position from current_end.shape[0]
...
class StreamingMultiheadAttention(StatefulModule):
    ...
    def init_state(self, batch_size: int, sequence_length: int) -> Dict[str, mx.array]:
        ...
        # Use empty array for current_end - position is tracked by shape[0]
    ...
    def increment_step(self, state: Dict[str, mx.array], increment: int = 1) -> None:
        ...
        """
        # Create new array with increased size - position is tracked by shape[0]
